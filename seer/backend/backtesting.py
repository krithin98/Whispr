...
class BacktestingEngine:
    ...
    async def get_historical_data(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            if data.empty:
                await log_event("backtest_warning", {
                    "message": f"No historical data found for {symbol} from {start_date} to {end_date}, using simulated data"
                })
            ...
            await log_event("backtest_info", {
                "message": f"Loaded {len(formatted_data)} days of real historical data for {symbol}",
                "symbol": symbol,
            ...
            await log_event("backtest_error", {"error": f"Failed to fetch historical data: {str(e)}"})
